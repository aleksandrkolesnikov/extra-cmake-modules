...
similar to the "qmltest" executable, but built locally so it respects local
compile flags. In addition, the four ``_CODE`` arguments allow injecting
...
The ``NAME`` argument sets the executable name. If it is not provided,
``ecm_qml_test_runner`` is used as default.
...
function(ecm_create_qml_test_runner)
    ...
    cmake_parse_arguments(PARSE_ARGV 0 ARG "" "${_single_args}" "")

    set(_name ${ARG_NAME})
    if ("${_name}" STREQUAL "")
        set(_name "ecm_qml_test_runner")
    endif()

    configure_file("${_ECM_QML_TEST_RUNNER_SOURCE}" "${CMAKE_CURRENT_BINARY_DIR}/ECMQmlTestRunner.cpp" @ONLY)

    add_executable(${_name} "${CMAKE_CURRENT_BINARY_DIR}/ECMQmlTestRunner.cpp")
    if (TARGET Qt::Qml)
        target_link_libraries(${_name} PRIVATE Qt::Qml Qt::QuickTest)
    else()
        target_link_libraries(${_name} PRIVATE Qt5::Qml Qt5::QuickTest)
    endif()
endfunction()
...
function(ecm_add_qml_test)
    ...
    if ("${_runner}" STREQUAL "")
        if (NOT TARGET ecm_qml_test_runner)
            message(FATAL_ERROR "ecm_add_qml_test called without RUNNER argument and default ecm_qml_test_runner not available")
        endif()
